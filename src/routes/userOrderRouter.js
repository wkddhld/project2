...
router.get('/', async (req, res, next) => {
  try {
    // 쿠키 확인
    const { userCookies, guestCookies } =
      req.cookies;
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      const { guestCookies, userCookies } =
        req.cookies;
      ...
      // 주문번호가 숫자가 아닌 경우
      if (
        !Number.isInteger(Number(orderNumber))
      ) {
        const err = new Error(
          '해당하는 주문 내역을 찾을 수 없습니다.',
        );
        err.statusCode = 404;
        return next(err);
      }
      ...
      for (const check of OrderCheck) {
        if (
          check.number === Number(orderNumber)
        ) {
          try {
            ...
            if (
              foundOrder.orderState === '배송완료'
            ) {
              const err = new Error(
                '배송 완료된 상품이라 주문 취소가 불가합니다.',
              );
              err.statusCode = 400;
              return next(err);
            }
