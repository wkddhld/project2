...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const productNumber =
        Date.now() + generateNumericOrderNumber();
      // 상품 정보 db에 저장
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      if (
        !Number.isInteger(Number(productNumber))
      ) {
        const err = new Error(
          '요청하는 상품을 찾을 수 없습니다.',
        );
...
router.delete(
  ...
  async (req, res, next) => {
    ...
    fs.unlinkSync(
      'src/productImages/' + foundData.image,
    );
