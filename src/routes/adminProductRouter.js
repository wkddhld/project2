...
router.post('/', upload.single('file'), async (req, res, next) => {
  try {
    ...
    const { name, price, information, origin, categoryName, subCategoryName } =
      req.body;
...
router.put('/:productNumber', upload.single('file'), async (req, res, next) => {
  try {
    ...
    const { name, price, information, categoryName, subCategoryName } =
      req.body;
    ...
    if (req.file) {
      ...
      // 변경하려는 이미지 업로드
      fs.linkSync(
        'src/productImages/' + foundProduct.image,
    ...
    if (data.modifiedCount === 0) {
      const err = new Error('업데이트 하려는 상품을 찾을 수 없습니다.');
      err.statusCode = 404;
      return next(err);
    }
